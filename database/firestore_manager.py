...
class FirestoreManager:
    ...
    def upload_exchange_rates(
        self, currencies: List[Currency], collection_name: str
    ) -> None:
        ...
        try:
            ...
            logger.info(
                f"Attempting to write data to Firestore. {collection_name}, {len(currencies)} entries. data size: {sys.getsizeof(exchange_data)} bytes."
            )
            self.db.collection(collection_name).document(current_date).set(
                exchange_data
            )
